...
export const suggestFoodPairing = ({ session }: SuggestFoodPairingProps) =>
  tool({
  description: 'Suggest wines from the user\'s cellar that pair well with a specific dish or food using wine pairing expertise.',
  parameters: z.object({
    ...
    limit: z.number().optional().default(8).describe('Maximum number of wine suggestions'),
  }),
  execute: async (args) => {
    if (!session?.user?.id) {
    ...
    try {
      ...
      const availableOptions = await getAvailableWineOptions({ userId: session.user.id });
...
Available wine types: ${availableOptions.types.filter(t => ['Red', 'White', 'Rosé', 'White - Sparkling'].includes(t)).join(', ')}
...
Limit to 3-4 varietals and 2-3 regions maximum.`;
      ...
      try {
        ...
        pairingRecommendations = {
          recommendedVarietals: ['Pinot Noir', 'Chardonnay', 'Sauvignon Blanc'],
          recommendedRegions: [],
          recommendedTypes: args.wineType ? [args.wineType] : ['Red', 'White'],
          reasoning: 'Using default pairing suggestions due to analysis error.'
        };
      }
      ...
      const typesToQuery = args.wineType ? [args.wineType] : pairingRecommendations.recommendedTypes || [];
      ...
      return {
        ...
        pairingAnalysis: {
          recommendedVarietals: pairingRecommendations.recommendedVarietals || [],
          recommendedRegions: pairingRecommendations.recommendedRegions || [],
          recommendedTypes: pairingRecommendations.recommendedTypes || [],
          reasoning: pairingRecommendations.reasoning || 'Wine pairing analysis completed.'
        },
